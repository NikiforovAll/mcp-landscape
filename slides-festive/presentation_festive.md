...
# Advanced MCPs for .NET: Registry, Security, and Agent Orchestration
...
# It is more than just an Web API!
...
# Demo: Claude Code Chrome MCP Server
...
1. Initialize the `server.json` file:
```bash
mcp-publisher init
```

2. Edit the `server.json` file to define your server (next slide).
...
2. **Client Registration**: Dynamic client registration with authorization server or Client ID Metadata Documents
